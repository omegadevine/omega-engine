...
// Console utility - FIXED coordinate system
class Console {
private:
...
public:
    ...
    static void clear() {
        ...
        FillConsoleOutputAttribute(hConsole, GRAY, size, coord, &written);
    }
...
class Dungeon {
public:
    static const int WIDTH = 60;  // Smaller, more manageable
    ...
    void generateDungeon() {
        // Create simple, well-spaced rooms
        ...
        for (int i = 0; i < numRooms; i++) {
            ...
            // Carve room - CRITICAL: tiles[y][x] not [x][y]!
    ...
    void draw() {
        ...
        // Draw grid with coordinates - tiles[y][x] matches screen correctly
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                char tile = tiles[y][x];
                int color = GRAY;
        ...
        // Draw controls with direction indicators
        ...
        string pos = "Your position: X=" + to_string(player->pos.x) + " Y=" + to_string(player->pos.y);
        Console::drawString(0, HEIGHT + 3, pos, CYAN);
    }
};

// Player move - FIXED: dx affects X (horizontal), dy affects Y (vertical)
...
public:
    ...
    void playLevel() {
        ...
        while (running && dungeon->player->alive && !dungeon->exitReached) {
            ...
            if (_kbhit()) {
                ...
                // CRITICAL: W/S affect Y (up/down), A/D affect X (left/right)
                if (key == 'w' || key == 'W') dungeon->player->move(0, -1, *dungeon);  // UP
                else if (key == 's' || key == 'S') dungeon->player->move(0, 1, *dungeon);   // DOWN
                else if (key == 'a' || key == 'A') dungeon->player->move(-1, 0, *dungeon);  // LEFT
                else if (key == 'd' || key == 'D') dungeon->player->move(1, 0, *dungeon);   // RIGHT
    ...
    void showTitle() {
        ...
        Console::drawString(20, 5, "====================================", BRIGHT_CYAN);
        Console::drawString(20, 6, "    OMEGA ENGINE - DUNGEON QUEST   ", BRIGHT_CYAN);
        Console::drawString(20, 7, "        V3.0 - GRID FIXED          ", BRIGHT_CYAN);
        Console::drawString(20, 8, "====================================", BRIGHT_CYAN);
